# Usage: CUDA_VISIBLE_DEVICES=gpu-number python deepSimDEF_for_sequence_homology.py arguments
...
from datasets import sequence_homology_dataset, generic_production_dataset
...
parser.add_argument('--sequence_homology_metric', default='LRBS', type=str, help='metric for sequence homology correlation evaluation ("RRBS" or "LRBS")')
...
def fit_sequence_homology(models, args):
    ...
    for e in range(checkpoint_baseline, args.nb_epoch):
        print("~~~~~~~~ {} ({}/{}) ~~~~~~~~ EPOCH {}/{} (Embedding dimention: {}) ~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n".format(
                '/'.join(sub_ontology_interested), args.species, args.sequence_homology_metric , e+1, args.nb_epoch, args.embedding_dim))
        if args.adaptive_lr:
        ...
        for model_index in range(len(models)): # Going through each model one by one
            ...
            if args.nb_fold!=1: # Only evaluation and report in n-fold cross-validation set up
                ...
                log_model_result_for_sequence_homology(e+1, model_index+1, learning_rate, 
        ...
        if args.nb_fold!=1: # Stats on all folds in this epoch
            log_epoch_result_for_sequence_homology(e+1, args.nb_epoch, current_iteration_elapsed, 
                             best_cv_pearson, pr_res, best_epoch_pearson, 
                             best_cv_spearman, sp_res, best_epoch_spearman, logdir)

def eval_sequence_homology(models, args):
...
if __name__ == "__main__": 
    ...
    if args.deepsimdef_mode=='training' or args.deepsimdef_mode=='evaluation':
        ...
        if args.inpute_file=='default':
            data_file_name = f'{args.species}_sequence_homology.tsv'
        else:
            data_file_name = args.inpute_file
        sequence_homology_data_dir = './data/species/{}/sequence_homology/processed'.format(args.species) # directory to the ppi datasets, pay attention to the file names and thier content format
        ...
        if args.checkpoint is None:
            args.log_name = f"{args.log_name}_{args.species}/{args.sequence_homology_metric}/pretrained_emb_{args.pretrained_embedding}_iea_{args.iea}_ontology_{args.sub_ontology}"
        ...
        gene_1, gene_2, gene_1_annotation, gene_2_annotation, prediction_value = sequence_homology_dataset(
            sequence_homology_data_dir, data_file_name, fully_annotated_genes, gene_annotations,
            gene_indeces, max_ann_len, args.sequence_homology_metric, args.partial_shuffle_percent, sub_ontology_interested)
        ...
        if args.deepsimdef_mode=='training':
            fit_sequence_homology(models, args)
            ...
            eval_sequence_homology(models, args)
